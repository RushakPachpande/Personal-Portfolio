  ...
  if (portfolioQuery.isError) {
    return (
      <div className="flex min-h-screen items-center justify-center px-6 text-center">
        <div>
          <h1 className="font-display text-2xl font-semibold">Unable to load portfolio data</h1>
          <p className="mt-3 max-w-md text-sm text-muted-foreground">
            The site could not reach Supabase. Confirm local `supabase start` or production project
            keys, then refresh.
          </p>
        </div>
      </div>
  ...
  return (
    <PortfolioContext.Provider value={portfolioQuery.data}>
      <DevModeContext.Provider value={{ unlocked }}>
        ...
        <main id="main-content" className="min-h-[70vh] min-w-0 overflow-x-clip">
