...
export function SurfaceCard({ title, gradient, className, header, children }: SurfaceCardProps) {
  return (
    <article
      className={cn(
    ...
    >
      {gradient ? (
        <div className={cn('h-1 w-full', getCardGradient(gradient))} aria-hidden="true" />
      ) : null}
      {header ?? (title ? (
