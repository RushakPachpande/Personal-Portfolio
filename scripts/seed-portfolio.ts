...
function isLocalAsset(value: string) {
  return path.isAbsolute(value) || value.startsWith('D:\\') || value.startsWith('/')
}
...
async function seedCaseStudies() {
  ...
  for (const [index, study] of caseStudies.entries()) {
    const logoPath = await resolveMedia(study.logo, `logos/${study.slug}${path.extname(study.logo ?? '.png')}`)
...
async function seedTechnologies() {
  ...
  for (const [index, technology] of technologies.entries()) {
    ...
    const logoPath = await resolveMedia(technology.logo, `tech/${technology.id}${ext}`)
    rows.push({
      id: technology.id,
...
async function maybeCreateAdmin() {
  ...
  if (!email || !password) {
    console.log('ADMIN_EMAIL / ADMIN_PASSWORD not set — skipping admin user create')
    return
  }
...
async function main() {
  ...
  const { error: timelineError } = await supabase.from('timeline_items').upsert(timelineRows)
  ...
  const { error: philosophyError } = await supabase.from('philosophy_pillars').upsert(philosophyRows)
  if (philosophyError) throw philosophyError

  await seedResume()

  await supabase.from('terminal_commands').delete().neq('command', '')
  const { error: terminalError } = await supabase.from('terminal_commands').insert(
